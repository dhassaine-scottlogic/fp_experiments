...
describe('filter, pick and sort data problem', function () {
    ...
    function fetchTaskData(): Promise<taskResponse> {
    ...
    */
    ...
    const expectedAnswer: Array<BasicTask> = [
        {
    ...
    describe('fp with some ramda', function () {
        function filterUserName(username: string) {
            return (tasks: Array<Task>) => tasks.filter((task) => task.username === username);
        }
        function filterOutstandingTasks(tasks: Array<Task>) : Array<BasicTask> {
            return tasks.filter((task) => !task.complete)
                .map(({id, title, dueDate, priority}) => Object.assign({}, { id, title, dueDate, priority }));
        }
        function descSort() {
            return (data: Array<any>) => data.sort((a, b) => a.localeCompare(b));
        }

        function getIncompleteTaskSummaries(username: string) {
            return R.pipeP(fetchTaskData, pluckTasks, filterUserData, filterOutstandingTasks, sortBydueDateDescending)();

            function filterUserData(data: Array<Task>) {
                return filterUserName(username)(data);
            }
            
            function sortBydueDateDescending(data: Array<BasicTask>) {
                return data.sort((a, b) => pluckTaskDate(a).localeCompare(pluckTaskDate(b)));
            }
            function pluckTaskDate(task: BasicTask) {
                 return task.dueDate;
            }
            function pluckTasks(httpResponse: taskResponse) {
